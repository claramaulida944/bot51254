...
class ProxyManager:
    ...
    def load_proxies(self) -> int:
        ...
        with self._lock:
            ...
            try:
                self._last_mtime = self.proxy_file.stat().st_mtime
            except Exception:
                pass
    ...
    def reload_if_modified(self) -> bool:
        """Mengecek apakah proxies.txt telah diedit atau ditambah proxy baru oleh owner."""
        try:
            if self.proxy_file.exists():
                mtime = self.proxy_file.stat().st_mtime
    ...
    def sync_from_hypeproxy(self, force: bool = False) -> int:
        """
                    ...
                    if has_changes:
                        ...
                        try:
                            ...
                            if root_file.exists() and root_file.resolve() != self.proxy_file.resolve():
                                HypeProxyClient.sync_proxies_to_file(output_file=str(root_file), user_only=True)
                        except Exception:
                            pass
                        self.load_proxies()
                        return len(self.parsed_proxies)
...
def test_proxy_cli() -> None:
    ...
    while True:
        ...
        n_slots = len(mgr.parsed_proxies)
        console.print(
            Panel(
                ...
                f"• [bold cyan]Load Balancing {n_slots} Slot:[/] Seluruh {n_slots} proxy Anda dibagi bergiliran (Round-Robin) ke tiap thread.\n\n"
        ...
        console.print(f"  [1] [bold green]One-Click Refresh & Rotasi Semua {n_slots} Proxy[/]")
        ...
        if choice == "0":
            ...
            # One-Click Refresh & Rotasi Semua Proxy
            console.print("\n[bold cyan]>>> Menyinkronkan daftar proxy dan merotasi IP seluruh slot...[/]")
            ...
            console.print(f"[bold green]✓ Seluruh {len(default_proxy_manager.parsed_proxies)} Slot Proxy HypeProxy berhasil disinkronkan & dirotasi IP baru![/]\n")
