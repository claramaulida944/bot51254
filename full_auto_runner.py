...
async def run_full_auto_cli(preset_target: Optional[str] = None) -> None:
    ...
    category_line = "\n[bold cyan]Kategori Konten:[/] [bold red]🔞 18+ Khusus Dewasa (Adult Only)[/]" if is_adult else ""
    ...
    # 2. Pemilihan Opsi Interaksi & Proteksi Anti-Duplikasi
    fresh_accounts, used_accounts = AccountHistoryManager.filter_fresh_accounts(accounts, novel_id)
    if used_accounts:
        console.print(f"[bold yellow][Anti-Duplikasi][/] [dim]{len(used_accounts)} akun dilewati otomatis karena sudah pernah berinteraksi (baca/like/simpan) dengan buku ini.[/]")

    if not fresh_accounts:
        console.print("[bold red][PERINGATAN][/] Seluruh akun di 'akun.txt' sudah pernah berinteraksi dengan buku ini!")
        return

    account_count = IntPrompt.ask(
        f"[bold green]?[/] Berapa akun segar yang ingin dijalankan? [1-{len(fresh_accounts)}]",
        default=len(fresh_accounts),
    )
    account_count = max(1, min(account_count, len(fresh_accounts)))
    selected_accounts = fresh_accounts[:account_count]
